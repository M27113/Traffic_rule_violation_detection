    ...
    def detect_image(self, image):
        ...
        for i, c in reversed(list(enumerate(out_classes))):
            ...
            if c == 0:
                ...
                cv2.putText(image, "Car", (int(left)-2, int(top)-2), cv2.FONT_HERSHEY_SIMPLEX, 0.4,(255,255,0),1)
               
            if c == 1:
                cv2.rectangle(image, (left, top), (right, bottom), (0,255,255), 1)
                #cv2.rectangle(image, (left, top), (right, bottom), (0,0,255), 1)
                cv2.putText(image, "Bike", (int(left)-2, int(top)-2), cv2.FONT_HERSHEY_SIMPLEX, 0.4,(0,255,255),1)
                #cv2.putText(image, "Person", (int(left)-2, int(top)-2), cv2.FONT_HERSHEY_SIMPLEX, 0.4,(0,0,255),1)
            ...
            #else:
             #   cv2.rectangle(image, (left, top), (right, bottom), (225,0,255), 1)
              #  cv2.putText(image, "Bike", (int(left), int(top)), cv2.FONT_HERSHEY_SIMPLEX, 0.4,(255,0,255),1)   
...
def detect_video(yolo, video_path, output_path=""):
    ...
    while True:    
        ...
        #frame = frame[int(0.01*height):height, int(0.05*width):int(2*width)]
            ...
            for i in range(len(tracker.tracks)):
                ...
                if(len(tracker.tracks[i].trace)>1): 
                    ...
                    if direction == "East":
                        ...
                        cv2.putText(result, 'WRONG DIRECTION', (int(x1)-20, int(y1)-5), cv2.FONT_HERSHEY_SIMPLEX, 0.30, (0, 0, 255), 1)
                        now = dt.datetime.now()
