...
from pyimagesearch.centroidtracker2 import CentroidTracker
ct = CentroidTracker()
@torch.no_grad()
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if ((ROI_MIN <= box_center <= ROI_MAX) and (int(cls)==1 or int(cls)==2 or int(cls)==3)): #In our dataset, only class 1 and 2 are vehicles
        ...
        objects, CY1, CY2 ,CX1 ,CX2 = ct.update(rects)  #send the box to tracker

        for (objectID, centroid) in objects.items():
            cy1=list(CY1.values())[objectID]
            cy2=list(CY2.values())[objectID]
            cx1=list(CX1.values())[objectID]
            cx2=list(CX2.values())[objectID]
            ...
            d_pixels = (cy2-cy1)*(cy2-cy1)
            d_pixel = math.sqrt(d_pixels)
            ppm = 50
            d_meters = d_pixel / ppm
            fps = 30
            speed = d_meters * fps * 3.6
            if (speed!= None):
                cv2.putText(im0, str(int(speed)) + " km/hr", (centroid[0], centroid[1]),cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
            if  (speed>60):
                cv2.putText(im0, 'OVERSPEED', (centroid[0] + 10, centroid[1] + 10),cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0,255), 2)
